...
/*
 * max98520.c  --  MAX98520 ALSA Soc Audio driver
 *
	...
	(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	pr_info("[MAX98520_DEBUG] %s sample ch = %d, width = %d\n", __func__, ch, width);
	
	ret = snd_soc_dai_set_tdm_slot(rtd->codec_dai, 1, 0x10, ch, width);
	if (ret < 0)
		return ret;

	pr_info("[MAX98520_DEBUG] %s codec tdm slot config ret : %d\n", __func__, ret);

	ret = snd_soc_dai_set_tdm_slot(rtd->cpu_dai, 0x03, 0x03, ch, width);
	if (ret < 0)
		return ret;

	pr_info("[MAX98520_DEBUG] %s cpu tdm slot config ret : %d\n", __func__, ret);
#endif

	pr_info("[MAX98520_DEBUG] %s sample rate = %d\n", __func__, sr);
	...
	pr_info("[MAX98520_DEBUG] %s codec ret : %d\n", __func__, ret);
	...
	pr_info("[MAX98520_DEBUG] %s cpu ret : %d\n", __func__, ret);
...
#endif
	pr_info("[MAX98520_DEBUG] %s out. ret:%d\n", __func__, ret);
	return ret;
}
...
SND_SOC_DAILINK_DEFS(audioinjector_maxim,
	...
	DAILINK_COMP_ARRAY(COMP_CODEC("max98520.1-0038", "max98520-aif1")),
...
static const struct snd_soc_dapm_route audioinjector_audio_map[] = {
	...
	/* mic is connected to Mic Jack, with max98520 Mic Bias */
...
static int audioinjector_maxim_soundcard_probe(struct platform_device *pdev)
{
	...
	pr_info("[MAX98520_DEBUG] %s --- ret : %d\n", __func__, ret);
	return ret;
}
